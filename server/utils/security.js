...
const onlyRole = (args) => (
	(req, res, next) => {
		...
		if (!args.includes(req.user.role) && !args.includes("any")) return res.json({
			...global.jsonBag,
		...
		if (args.includes(`${req.user.role}::account`)) {
			if (typeof req.user.profile?._id == "undefined") return res.json({
				...global.jsonBag,
				status: res.statusCode = 403, // Forbidden
				error: {
					message: `Sorry, you don't have ${req.user.role == 'employer' ? "an employer's" : "a candidate's"} account. Go create one!`,
					data: null,
				},
				data: null
			})
...
const authUserByPassword = async (req, res, next) => {
	...
	try {
		const logged_in_user = await User.findById(req.user._id, ["password"]).lean()
		if (await bcrypt.compare(auth_password, logged_in_user.password)) return next()
